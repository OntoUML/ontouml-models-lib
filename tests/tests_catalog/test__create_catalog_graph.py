...
def test_create_catalog_graph_consistency(ontouml_models_path: Path) -> None:
    ...
    for model in catalog.models:
        ...
        assert (model_id, None, None) in graph or (
            None,
            None,
            model_id,
        ) in graph, f"Expected the graph to contain references to model: {model.id}"
...
def test_catalog_graph_contains_expected_triples(ontouml_models_path: Path) -> None:
    ...
    expected_triples = [
        (
            URIRef("https://w3id.org/ontouml-models/model/abel2015petroleum-system/"),
            URIRef(RDF.type),
            URIRef("https://w3id.org/ontouml#Project"),
        ),
    ]
...
def test_create_catalog_graph_after_model_removal(ontouml_models_path: Path) -> None:
    ...
    assert (model_id, None, None) not in updated_graph and (
        None,
        None,
        model_id,
    ) not in updated_graph, f"Expected the graph to not contain references to removed model: {model_to_remove.id}"
